...
build() {
  ...
  if [ "$TRACK" = "$TRACK_DEBUG" ]; then
    ...
    OUTPUT_FILE_PATH="$OUTPUT_VERSION_DIR/kaizen-keyboard-$BUILD_VERSION_NAME.$TYPE"
  else
    OUTPUT_VERSION_DIR="$OUTPUT_DIR/$BUILD_VERSION_NAME-$TRACK"
    OUTPUT_FILE_PATH="$OUTPUT_VERSION_DIR/kaizen-keyboard-$BUILD_VERSION_NAME-$TRACK.$TYPE"
  fi
