...
package com.kaizen.keyboard.app.setup
...
import com.kaizen.keyboard.lib.compose.FlorisScreenScope
...
fun SetupScreen() = FlorisScreen {
    ...
    content(
        isKaizen_KeyboardEnabled,
        isKaizen_KeyboardSelected,
        context,
        navController,
        requestNotification,
...
private fun FlorisScreenScope.content(
    isKaizen_KeyboardEnabled: Boolean,
    isKaizen_KeyboardSelected: Boolean,
    context: Context,
    ...
    val stepState = rememberSaveable(saver = FlorisStepState.Saver) {
        val initStep = when {
            !isKaizen_KeyboardEnabled -> Steps.EnableIme.id
            !isKaizen_KeyboardSelected -> Steps.SelectIme.id
    ...
    content {
        LaunchedEffect(isKaizen_KeyboardEnabled, isKaizen_KeyboardSelected, hasNotificationPermission) {
...
private fun footer(context: Context) {
    ...
    Row(
        ...
        val privacyPolicyUrl = stringRes(R.string.kaizen_keyboard__privacy_policy_url)
        TextButton(onClick = { context.launchUrl(privacyPolicyUrl) }) {
            Text(text = stringRes(R.string.setup__footer__privacy_policy))
        }
        FlorisBulletSpacer()
        val repositoryUrl = stringRes(R.string.kaizen_keyboard__repo_url)
